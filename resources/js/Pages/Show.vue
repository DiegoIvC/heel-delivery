<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="bg-white p-4 rounded-lg shadow-lg max-w-3xl mx-auto mt-5">
            <button @click="regresar" class="bg-gray-600 text-white rounded-lg py-2 px-4 text-lg mb-5 flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 mr-2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 15 3 9m0 0 6-6M3 9h12a6 6 0 0 1 0 12h-3" />
                </svg>
                Regresar
            </button>
            <div class="flex flex-col lg:flex-row items-start space-y-4 lg:space-y-0 lg:space-x-6 p-6 bg-white shadow-md rounded-lg">
                <!-- Detalles de la Orden -->
                <div class="flex-1">
                    <h1 class="text-2xl lg:text-3xl font-extrabold text-gray-900 mb-6">Detalles de la Orden</h1>

                    <div class="space-y-4">
                        <div class="flex flex-col lg:flex-row items-start lg:items-center">
                            <span class="text-base lg:text-lg font-semibold text-gray-700 mr-2">Cliente:</span>
                            <p class="text-gray-800">{{ orden.cliente }}</p>
                        </div>

                        <div class="flex flex-col lg:flex-row items-start lg:items-center">
                            <span class="text-base lg:text-lg font-semibold text-gray-700 mr-2">Dirección:</span>
                            <p class="text-gray-800">{{ orden.direccion }}</p>
                        </div>

                        <div class="flex flex-col lg:flex-row items-start lg:items-center">
                            <span class="text-base lg:text-lg font-semibold text-gray-700 mr-2">Costo de entrega:</span>
                            <p class="text-gray-800">{{ costoEntregaFixed }}</p>
                        </div>

                        <div class="flex flex-col lg:flex-row items-start lg:items-center">
                            <span class="text-base lg:text-lg font-semibold text-gray-700 mr-2">Repartidor:</span>
                            <p class="text-gray-800">{{ orden.repartidor_nombre || 'Sin repartidor asignado' }}</p>
                        </div>

                        <div class="flex flex-col lg:flex-row items-start lg:items-center">
                            <span class="text-base lg:text-lg font-semibold text-gray-700 mr-2">Zona:</span>
                ...
                <div class="flex items-center">
                    <button @click="confirmAndFinish" class="bg-red-600 text-white rounded-lg text-lg lg:text-3xl py-2 px-4 mt-1">
                        PEDIDO PREPARADO
                    </button>
                </div>
            </div>
            ...
            <div class="overflow-x-auto mt-4">
                <table class="min-w-full bg-white border border-gray-200 rounded-lg">
                    ...
                    <tr>
                        <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Hamburguesa</th>
                    ...
                    <tr v-for="detalle in detalles" :key="detalle.id">
                        <td class="px-4 py-2 text-sm lg:text-base text-gray-900">{{ detalle.nombre_hamburguesa }}</td>
                        <td class="px-4 py-2 text-sm lg:text-base text-gray-900">{{ detalle.cantidad }}</td>
                        <td class="px-4 py-2 text-sm lg:text-base text-gray-500">{{ detalle.precio_unitario }}</td>
                        <td class="px-4 py-2 text-sm lg:text-base text-gray-500">{{ detalle.precio_total }}</td>
                    </tr>
                    </tbody>
                </table>
            </div>
            ...
            <button @click="confirmAndEdit" class="mt-4 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded flex items-center justify-center">
                <div class="text-sm lg:text-base font-medium mr-2">Editar Orden</div>
